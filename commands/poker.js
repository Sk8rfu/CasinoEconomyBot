...
function evaluateHand(hand) {
    ...
    if (isStraight && isFlush && values.includes("A") && values.includes("K")) return {name:"Royal Flush", mult:100};
...
.setName('poker')
.setDescription('Играй 5-карти покер срещу дилъра')
...
export async function execute(interaction) {
    ...
    if (pEval.mult > dEval.mult) {
        ...
        result = `🎉 Победи с **${pEval.name}**!`;
    } else if (pEval.mult < dEval.mult) {
        result = `💀 Загуби. Дилърът имаше **${dEval.name}**.`;
    } else {
        win = bet;
    ...
    .addFields(
        { name: 'Твоите карти', value: `${player.join(" ")}`, inline: false },
               { name: 'Комбинация', value: pEval.name, inline: true },
               { name: 'Карти на дилъра', value: `${dealer.join(" ")}`, inline: false },
               { name: 'Комбинация на дилъра', value: dEval.name, inline: true },
               { name: 'Резултат', value: result, inline: false }
    )
