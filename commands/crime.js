...
.setName('crime')
.setDescription('Опитай да извършиш престъпление за пари');
...
export async function execute(interaction) {
    ...
    if (last && now - last < CRIME_COOLDOWN) {
        ...
        return interaction.reply(`⏳ Изчакай още **${minutes} минути**.`);
    }
    ...
    if (success) {
        ...
        return interaction.reply(`🦹 Успех! Спечели **${reward} лв**.`);
    } else {
        ...
        return interaction.reply(`🚨 Провал! Полицията те хвана и плати глоба **${fine} лв**.`);
    }
}
